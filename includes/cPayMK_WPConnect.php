...
class cPayMKGiveWPConnect
{
	...
	const PRODUCTION_URL = 'https://www.cpaymk.com/api/';
	const STAGING_URL = 'https://www.cpaymk-sandbox.com/api/';
	...
	public static function getXSignature($x_signature_key) {
		...
		if (isset($_GET['cpaymk']['x_signature'])) {
			$keys = array('id', 'paid_at', 'paid', 'transaction_id', 'transaction_status', 'x_signature');

			foreach ($keys as $key){
				if (isset($_GET['cpaymk'][$key])){
						$data['cpaymk'][$key] = $_GET['cpaymk'][$key];
				}
			}
			$type = 'redirect';
		}
